...
| C | Queue awareness: owed frontiers never jump past an unrecovered queued window; disjoint queued windows do not clamp a request away (e8019d6); a queued window that does not COVER the frontier never skips the uncovered prefix, and chained coverage is order-independent for the non-chronological queue (PR #81 rounds 2–3) | `BackfillRecovery.owedWindow`, `BackfillQueue.clamp` | `BackfillRecoveryTest` + `BackfillQueueTest` — overlapping raise, disjoint ignored, subsumed → null, max over chained coverage, partial-overlap prefix preserved, order-independence, uncovered gap kept |
...
(60 tests total) written first and confirmed failing against `TODO`
stubs (the red run recorded 212 completed / 51 failed; edge tests and the
PR #81 partial-overlap / order-independence regressions were added during
review), then green.
...
`gradle :core:test` (221 tests, 0 failures; includes the PR #81
partial-overlap and order-independence regressions),
`gradle :app:testDebugUnitTest` (179 tests, 0 failures; 400 total),
...
- **Claim-time union of a preserved prefix (PR #81 review, follow-up)**:
  `clamp`/`owedWindow` now sweep the queued windows by start, so chained
  coverage is order-independent (the queue is explicitly not chronological)
  and a queued window starting inside a span no longer drops the uncovered
  prefix at the decision layer (regression-tested).
